import numpy as np
from sklearn.metrics import accuracy_score, roc_auc_score, f1_score, recall_score, confusion_matrix, precision_score
...
def pretext_generator(m, x):
    ...
    Returns:
      m_new: final mask matrix after perturbation
      x_tilde: perturbed feature matrix
    """
...
def perf_metric(metric, y_test, y_test_hat):
    ...
    if metric == 'acc':
        ...
        y_p = np.argmax(y_test_hat, axis = 1)
    ...
    # Specificity score
        ...
        tn, fp, fn, tp = confusion_matrix(np.argmax(y_test, axis=1),
                                          np.argmax(y_test_hat, axis=1)).ravel()
        result = tn / (tn + fp)

    # precision score
    elif metric == 'precision':
        result = precision_score(np.argmax(y_test, axis=1),
                                 np.argmax(y_test_hat, axis=1))
    return result
